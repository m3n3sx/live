...
# Exclude node_modules, .git, .vscode, and the script itself
find . -type f \
  -not -path "./.git/*" \
  -not -path "./node_modules/*" \
  -not -path "./.vscode/*" \
  -not -path "./create_gist.sh" \
  -not -path "./create_gist_v2.sh" \
  -not -path "./cursor-localhost-viewer/node_modules/*" | while read file; do
